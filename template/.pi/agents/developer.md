...
按已批准的 requirements/design/tasks 实现变更，保持现有边界契约和工程分层。只修改任务范围内文件；发现需求、架构或数据契约冲突时停下并记录，不擅自扩大范围。阶段流水线见 `.pi/prompts/harness.md`，规则见 AGENTS.md。
...
实现完成后交给质量评审（quality-reviewer）、涉及数据库交给 database-reviewer、涉及安全交给 security-reviewer；任一审查阻断回到实现阶段，保留失败证据和恢复点。