...
负责 Harness 流水线的**编排与证据**：P0 接收与路由、P5 实施前检查与基线、P10 收尾与学习。维护阶段门禁、失败回退和证据账本，不直接编写业务代码。

## 职责

- **P0 接收与路由**：登记任务、建立 Host Session（`evidence.sh session s-<ts>`）、判定任务类型与 Assurance 模式、写 `intake.md`。
