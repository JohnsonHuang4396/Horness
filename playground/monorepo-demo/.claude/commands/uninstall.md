...
| 模式 | 动作 |
|---|---|
| `detach` | 停用：移除 `.harness/current-session` 指针与运行事件，保留 `.claude/` 与规则 |
| `clean-history` | 清理运行记录：清空 `.harness/host-sessions/` 与 `.harness/generated/` 运行时状态 |
| `uninstall` | 完整卸载：移除 `.claude/` 的 agents/skills/commands/hooks 与 `.harness/` |

## 步骤

1. **预览（preview）**：扫描 `.claude/` 与 `.harness/` 所有资产，按以下分类：
   - **删除（Harness 生成物）**：`.harness/` 运行时账本、`.claude/commands` 的 harness 命令、`.claude/settings.json` 的 hook。
   - **备份（用户修改过的）**：用户编辑过的 AGENTS.md 工作规范、`.claude/settings.local.json`。
   - **保留（用户资产）**：业务代码、CONTEXT.md、HANDOFF.md、用户自建规则。
   - **需确认**：`.claude/skills/`、`.claude/agents/`（可能是用户自建，需逐项确认）。
