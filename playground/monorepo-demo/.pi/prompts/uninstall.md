...
| 模式 | 动作 |
|---|---|
| `detach` | 停用：移除 `.harness/current-session` 指针与运行事件，保留 `.pi/`、`.claude/` 与规则 |
| `clean-history` | 清理运行记录：清空 `.harness/host-sessions/` 与 `.harness/generated/` 运行时状态 |
| `uninstall` | 完整卸载：移除 `.pi/` 的 agents/prompts/extensions、`.claude/` 的 agents/skills/commands/hooks 与 `.harness/` |

## 步骤

1. **预览（preview）**：扫描 `.pi/`、`.claude/` 与 `.harness/` 所有资产，按以下分类：
   - **删除（Harness 生成物）**：`.harness/` 运行时账本、`.pi/prompts` 的 harness 命令 pawn、`.pi/settings.json`、`.claude/commands` 的 harness 命令、`.claude/settings.json` 的 hook。
   - **备份（用户修改过的）**：用户编辑过的 AGENTS.md 工作规范、`.claude/settings.local.json`。
   - **保留（用户资产）**：业务代码、CONTEXT.md、HANDOFF.md、用户自建规则。
   - **需确认**：`.pi/agents/`、`.claude/skills/`、`.claude/agents/`（可能是用户自建，需逐项确认）。
...
- 不删除用户文件、旧数据、冲突源文件或用户自建 Skill/规则。
- 删除前必须备份；有回滚路径。
- 不读取/打印/提交秘密；卸载报告脱敏。