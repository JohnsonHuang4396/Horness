...
2. **candidate**：生成唯一 `proposal_id`、根因、证据路径、目标文件、优先级、影响面、回滚点，追加到 `.harness/generated/evolution-candidates.json`。
...
- 每条根因：问题来源、问题现象、问题类型、harness 根因、影响范围。

## 候选必含字段

`proposal_id`、`root_cause`、`evidence`、`target_file`（`.claude/` 或 `.harness/` 下）、`priority`（P0/P1/P2）、`impact`、`before_hash`、`after_hash`、`eval`、`approval`、`state`（candidate|eval|approved|canary|installed|rolled-back）、`rollback`、`unknowns`。无证据（缺 source/facts_hash/hash/评测/审批者/回滚/unknowns）的建议只能显示为「待补证据」，不能进入 active。
